...
public class Main extends ApplicationAdapter {
	...
	public void render() {
		...
		sr.rectLine(cueBall.getX(), cueBall.getY(), cursor.getX(), cursor.getY(), 3);
	...
	public boolean checkPocketCollision(Pocket pocket, Ball ball2) {
		float dx = pocket.getX() - ball2.getX();
		float dy = pocket.getY() - ball2.getY();
		float distance = (float) Math.sqrt((dx * dx) + (dy * dy));
		return distance <= pocket.getR() + ball2.getR();
	}
	...
	public void shotPredictor(){
		float gradLine = ((cueBall.getY() - cursor.getY()) / (cueBall.getX() - cursor.getX()));
		System.out.println(gradLine);
    }
