...
public class Main extends ApplicationAdapter {
	...
	Ball[] simSet11 = new Ball[15];
	Ball[] simSet12 = new Ball[15];
	Ball[] simSet13 = new Ball[15];
	Ball[] simSet14 = new Ball[15];
	Ball[] simSet15 = new Ball[15];

	ArrayList<Ball[]> simSets = new ArrayList<>();
	...
	public void create() {
		...
		simSets.add(simSet8);
		simSets.add(simSet9);
		simSets.add(simSet10);
		simSets.add(simSet11);
		simSets.add(simSet12);
		simSets.add(simSet13);
		simSets.add(simSet14);
		simSets.add(simSet15);

		for(int i = 0; i<15; i++){
			initializeBallsForTable(Sim[i]);
		}

		System.out.println(simSets.get(1)[1]);
	...
	*/
	...
	private void initializeBallsForTable(Table table) {
		int x = table.getX() + table.getW() / 2;
		int y = table.getY() + table.getH() / 2;
		int j = 0;
	...
	for(int k = 0; k<16; k++) {
		for (int i = 0; i < 15; i++) {
			if (i == 5 || i == 9 || i == 12 || i == 14) {
				...
				y = y + j * 12;
				if (i % 2 == 0) {
					simSets.get(i)[j] = new Ball(x, y, 9, 0, 0, false, Color.YELLOW);
				} else {
					simSets.get(i)[j] = new Ball(x, y, 9, 0, 0, false, Color.RED);
				}
			} else {
				y = y + 25;
				if (i % 2 == 0) {
