...
public class Main extends ApplicationAdapter {
	...
	public void create() {
		...
		cueBallSim1 = new Ball(300, 500, 6.75f, 0, 0, false, Color.WHITE);
		cueBallSim2 = new Ball(300, 500, 6.75f, 0, 0, false, Color.WHITE);
		cueBallSim3 = new Ball(300, 500, 6.75f, 0, 0, false, Color.WHITE);
		cueBallSim4 = new Ball(300, 500, 6.75f, 0, 0, false, Color.WHITE);
		cueBallSim5 = new Ball(300, 500, 6.75f, 0, 0, false, Color.WHITE);
		cueBallSim6 = new Ball(300, 500, 6.75f, 0, 0, false, Color.WHITE);
		cueBallSim7 = new Ball(300, 500, 6.75f, 0, 0, false, Color.WHITE);
		cueBallSim8 = new Ball(300, 500, 6.75f, 0, 0, false, Color.WHITE);
		cueBallSim9 = new Ball(300, 500, 6.75f, 0, 0, false, Color.WHITE);
		cueBallSim10 = new Ball(300, 500, 6.75f, 0, 0, false, Color.WHITE);
		cueBallSim11 = new Ball(300, 500, 6.75f, 0, 0, false, Color.WHITE);
		cueBallSim12 = new Ball(300, 500, 6.75f, 0, 0, false, Color.WHITE);
		cueBallSim13 = new Ball(300, 500, 6.75f, 0, 0, false, Color.WHITE);
		cueBallSim14 = new Ball(300, 500, 6.75f, 0, 0, false, Color.WHITE);
		cueBallSim15 = new Ball(300, 500, 6.75f, 0, 0, false, Color.WHITE);
	...
	public void render() {
		...
		simShots(ai.RandomPower(), ai.RandomRotation());


		for(int i = 0; i<15; i++){
			simBallMovement(ballSimSets.get(i), cueBalls.get(i));
		}
	...
	*/
	...
	public void initialiseCueBalls(Table table, Ball cueBall) {
		// Set the cue ball position to the center of the table
		float baseX =(table.getX() + (table.getW() * 0.25f));
		float baseY = (table.getY() + table.getH() / 2);
		cueBall.setX(baseX);
		cueBall.setY(baseY);
	}


	public void simShots(double power, double angle) {
		...
		for(int k = 0; k < 15; k++) {
			...
			if (checkBallsStationary()) {
				...
				cueBalls.get(k).setxVel(-(float) (power * Math.cos(angle)));
				cueBalls.get(k).setyVel(-(float) (power * Math.sin(angle)));

			}

			for (int i = 0; i < 15; i++) {
				for (int j = i + 1; j < 15; j++) {
					if (checkBallCollision(ballSimSets.get(k)[i], ballSimSets.get(k)[j])) {

						float relativeVelX = ballSimSets.get(k)[j].getxVel() - ballSimSets.get(k)[i].getxVel();
						float relativeVelY = ballSimSets.get(k)[j].getyVel() - ballSimSets.get(k)[i].getyVel();

						float collisionNormalX = ballSimSets.get(k)[j].getX() - ballSimSets.get(k)[i].getX();
						float collisionNormalY = ballSimSets.get(k)[j].getY() - ballSimSets.get(k)[i].getY();
						float collisionNormalLength = (float) Math.sqrt(collisionNormalX * collisionNormalX + collisionNormalY * collisionNormalY);
						collisionNormalX /= collisionNormalLength;
						collisionNormalY /= collisionNormalLength;
						...
						float dotProduct = relativeVelX * collisionNormalX + relativeVelY * collisionNormalY;
						float impulse = dotProduct; // Assuming same mass for both ballSimSets.get(k)

						ballSimSets.get(k)[i].setxVel(ballSimSets.get(k)[i].getxVel() + impulse * collisionNormalX);
						ballSimSets.get(k)[i].setyVel(ballSimSets.get(k)[i].getyVel() + impulse * collisionNormalY);
						ballSimSets.get(k)[i].setX(ballSimSets.get(k)[i].getX() - collisionNormalX);
						ballSimSets.get(k)[i].setY(ballSimSets.get(k)[i].getY() - collisionNormalY);
						ballSimSets.get(k)[j].setxVel(ballSimSets.get(k)[j].getxVel() - impulse * collisionNormalX);
						ballSimSets.get(k)[j].setyVel(ballSimSets.get(k)[j].getyVel() - impulse * collisionNormalY);
						ballSimSets.get(k)[j].setX(ballSimSets.get(k)[j].getX() + collisionNormalX);
						ballSimSets.get(k)[j].setY(ballSimSets.get(k)[j].getY() + collisionNormalY);
					}
				}
			}
		}
	...
	public void simBallMovement(Ball[] balls, Ball cueBall){
			for (int i = 0; i < 15; i++) {
	...
	public void simEdgeCollision(){
		for(int j =0; j<15; j++) {
			if (cueBalls.get(j).getY() + cueBalls.get(j).getR() >= Sim[j].getTableEdge3Y()) {
				cueBalls.get(j).setyVel(-(cueBalls.get(j).getyVel()));
				cueBalls.get(j).setY(Sim[j].getTableEdge3Y() - cueBalls.get(j).getR());
				cueBalls.get(j).setxVel((float) (0.85 * cueBalls.get(j).getxVel()));
				cueBalls.get(j).setyVel((float) (0.85 * cueBalls.get(j).getyVel()));
			}

			if (cueBalls.get(j).getY() - cueBalls.get(j).getR() <= Sim[j].getTableEdge2Y() + Sim[j].getTableEdge2H()) {
				cueBalls.get(j).setyVel(-(cueBalls.get(j).getyVel()));
				cueBalls.get(j).setY(Sim[j].getTableEdge2Y() + Sim[j].getTableEdge2H() + cueBalls.get(j).getR());
				cueBalls.get(j).setxVel((float) (0.85 * cueBalls.get(j).getxVel()));
				cueBalls.get(j).setyVel((float) (0.85 * cueBalls.get(j).getyVel()));
			}

			if (cueBalls.get(j).getX() + cueBalls.get(j).getR() >= Sim[j].getTableEdge1X()) {
				cueBalls.get(j).setxVel(-(cueBalls.get(j).getxVel()));
				cueBalls.get(j).setX(Sim[j].getTableEdge1X() - cueBalls.get(j).getR());
				cueBalls.get(j).setxVel((float) (0.85 * cueBalls.get(j).getxVel()));
				cueBalls.get(j).setyVel((float) (0.85 * cueBalls.get(j).getyVel()));
			}

			if (cueBalls.get(j).getX() - cueBalls.get(j).getR() <= Sim[j].getTableEdge0X() + Sim[j].getTableEdge0W()) {
				cueBalls.get(j).setxVel(-(cueBalls.get(j).getxVel()));
				cueBalls.get(j).setX(Sim[j].getTableEdge0X() + Sim[j].getTableEdge0W() + cueBalls.get(j).getR());
				cueBalls.get(j).setxVel((float) (0.85 * cueBalls.get(j).getxVel()));
				cueBalls.get(j).setyVel((float) (0.85 * cueBalls.get(j).getyVel()));
			}
		}
		...
		for(int k = 0; k<15; k++) {
			for (int i = 0; i < 15; i++) {
				if (ballSimSets.get(k)[i].getY() + ballSimSets.get(k)[i].getR() >= Sim[k].getTableEdge3Y()) {
					ballSimSets.get(k)[i].setyVel(-(ballSimSets.get(k)[i].getyVel()));
					ballSimSets.get(k)[i].setY(Sim[k].getTableEdge3Y() - ballSimSets.get(k)[i].getR());
					ballSimSets.get(k)[i].setxVel((float) (0.85 * ballSimSets.get(k)[i].getxVel()));
					ballSimSets.get(k)[i].setyVel((float) (0.85 * ballSimSets.get(k)[i].getyVel()));
				}

				if (ballSimSets.get(k)[i].getY() - ballSimSets.get(k)[i].getR() <= Sim[k].getTableEdge2Y() + Sim[k].getTableEdge2H()) {
					ballSimSets.get(k)[i].setyVel(-(ballSimSets.get(k)[i].getyVel()));
					ballSimSets.get(k)[i].setY(Sim[k].getTableEdge2Y() + Sim[k].getTableEdge2H() + ballSimSets.get(k)[i].getR());
					ballSimSets.get(k)[i].setxVel((float) (0.85 * ballSimSets.get(k)[i].getxVel()));
					ballSimSets.get(k)[i].setyVel((float) (0.85 * ballSimSets.get(k)[i].getyVel()));
				}

				if (ballSimSets.get(k)[i].getX() + ballSimSets.get(k)[i].getR() >= Sim[k].getTableEdge1X()) {
					ballSimSets.get(k)[i].setxVel(-(ballSimSets.get(k)[i].getxVel()));
					ballSimSets.get(k)[i].setX(Sim[k].getTableEdge1X() - ballSimSets.get(k)[i].getR());
					ballSimSets.get(k)[i].setxVel((float) (0.85 * ballSimSets.get(k)[i].getxVel()));
					ballSimSets.get(k)[i].setyVel((float) (0.85 * ballSimSets.get(k)[i].getyVel()));
				}

				if (ballSimSets.get(k)[i].getX() - ballSimSets.get(k)[i].getR() <= Sim[k].getTableEdge0X() + Sim[k].getTableEdge0W()) {
					ballSimSets.get(k)[i].setxVel(-(ballSimSets.get(k)[i].getxVel()));
					ballSimSets.get(k)[i].setX(Sim[k].getTableEdge0X() + Sim[k].getTableEdge0W() + ballSimSets.get(k)[i].getR());
					ballSimSets.get(k)[i].setxVel((float) (0.85 * ballSimSets.get(k)[i].getxVel()));
					ballSimSets.get(k)[i].setyVel((float) (0.85 * ballSimSets.get(k)[i].getyVel()));
				}
			}
