	...
	public void mouseCheck() {
		...
		for (int i = 0; i < 15; i++) {
			for (int j = 0; j < 15; j++) {
				if (checkBallCollision(balls[i], balls[j])) {
					...
					float ballJX = balls[i].getX();
					float ballJY = balls[i].getY();

					float ballIX = balls[j].getX();
					float ballIY = balls[j].getY();

					float distanceX = (ballIX - ballJX);
					float distanceY = (ballIY - ballJY);



					balls[i].setyVel(-distanceY * 5);
					balls[i].setxVel(-distanceX * 5);
				}
			}
		}
		...
		for (int i = 0; i < 15; i++) {
			if (checkBallCollision(balls[i], cueBall)) {
				float ballJX = (float) cueBall.getX();
				float ballJY = (float) cueBall.getY();

				float ballIX = (float) balls[i].getX();
				float ballIY = (float) balls[i].getY();

				float axisX = (ballJX - ballIX);
				float axisY = (ballJY - ballIY);

				float distance = (float) (Math.sqrt(axisX * axisX) + (axisY + axisY));

				if(distance < balls[i].getR() + cueBall.getR()){
					balls[i].setyVel(-(axisX / distance));
					balls[i].setxVel(-(axisY / distance));
